...
export type ClientId = "claude" | "gemini" | "cursor";
...
function buildEntry(apiKey: string, baseUrl: string) {
    return {
        command: "npx",
...
export function runInstall(clientId: string, apiKey: string, baseUrl: string) {
    ...
    const client = CLIENTS[clientId as ClientId];
    if (!client) {
