...
if (process.argv[2] === "install") {
    ...
    const opts = installCmd.opts<{ client: string; apiKey: string; baseUrl: string }>();
    runInstall(opts.client as any, opts.apiKey, opts.baseUrl);
    process.exit(0);
}
...
const program = new Command()
    ...
    .option(
        "--base-url <url>",
        "Base URL of your Log Pose instance (e.g., https://dashboard.mysite.com)"
    )
    .allowUnknownOption()
    .parse(process.argv);
...
if (resolvedBaseUrl) setBaseUrl(resolvedBaseUrl);

const server = new McpServer(
    { name: "Log Pose MCP", version: "1.0.1" },
    {
